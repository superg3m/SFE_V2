...
bool PlatformAPI::file_exists(const char* path) {
	return (GetFileAttributesA(path) != INVALID_FILE_ATTRIBUTES);
}

bool PlatformAPI::copy_file(const char* source_path, const char* dest_path, bool block_until_success) {
...
void PlatformAPI::sleep(u32 ms) {
	Sleep((DWORD)ms);
}

double PlatformAPI::get_seconds_elapsed() {
...
u8* PlatformAPI::read_entire_file(Allocator a, const char* file_path, size_t& out_file_size, Error& error) {
...
DLL* PlatformAPI::load_dll(const char* dll_path, Error& error)  {
...
DLL* PlatformAPI::free_dll(DLL* dll)  {
	RUNTIME_ASSERT(dll);
...
void* PlatformAPI::get_function_address(DLL* dll, const char* proc_name) {
	RUNTIME_ASSERT(dll);
...
FileTime PlatformAPI::get_file_modification_time(const char* file_path) {
	HANDLE file_handle = CreateFileA(
		file_path, 
		GENERIC_READ, 
		FILE_SHARE_READ|FILE_SHARE_WRITE|FILE_SHARE_DELETE, 
...
bool PlatformAPI::compare_file_modification_time(FileTime a, FileTime b) {
...
Allocator PlatformSystem::get_allocator() {
	Allocator allocator = {};
